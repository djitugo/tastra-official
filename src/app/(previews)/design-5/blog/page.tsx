          ...
          <div className="w-full max-w-[1700px] mx-auto px-5 sm:px-8 lg:px-16 py-16 md:py-20">
            <Reveal>
              <Link href={`/design-5/blog/${featured.slug}`} className="group grid md:grid-cols-2 gap-10 lg:gap-16 items-center">
                <div className="relative aspect-[4/3] overflow-hidden transition-transform duration-700 group-hover:scale-[1.02]" style={{ background: BEIGE_SOFT }}>
                  <PlaceholderGraphic bg="#f7f2e8" fg="#0a0a0a" label={featured.category} />
                </div>
                <div>
            ...
            {rest.map((post) => (
              <RevealItem key={post.slug}>
                <Link href={`/design-5/blog/${post.slug}`} className="group block">
                  <div className="relative aspect-[4/3] mb-5 overflow-hidden transition-transform duration-700 group-hover:scale-[1.02]" style={{ background: BEIGE_SOFT }}>
