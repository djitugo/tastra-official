    ...
    <>
      <section className="border-b-2 border-black">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          ...
          <h1 className="font-[family-name:var(--font-display)] uppercase text-6xl md:text-8xl mt-3 tracking-tighter">Keranjang.</h1>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          {!ready ? (
            ...
            <div className="border-2 border-black p-12 text-center">
              <p className="font-[family-name:var(--font-display)] uppercase text-3xl mb-3 tracking-tighter">Masih kosong.</p>
              <p className="text-black/70 mb-6">
                Mulai dari satu produk, atau ambil paketnya — hemat sampai 50%.
              </p>
              <Link href="/design-1#shop" className="btn btn-invert">Mulai belanja →</Link>
            </div>
          ) : (
            <div className="grid lg:grid-cols-[1fr_400px] gap-8">
              <ul className="border-2 border-black divide-y-2 divide-black">
                {items.map(({ line, product }) => (
                  <li key={line.slug} className="p-4 sm:p-6 flex gap-4 sm:gap-6">
                    <Link
                      href={`/design-1/shop/${product.slug}`}
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        <Link href={`/design-1/shop/${product.slug}`} className="font-[family-name:var(--font-display)] uppercase text-xl sm:text-2xl tracking-tight hover:underline">
                          {product.name}
                        </Link>
                        <p className="font-[family-name:var(--font-mono)] text-xs uppercase tracking-widest text-black/60 mt-1">
                      ...
                      <div className="flex items-end justify-between flex-wrap gap-3 mt-3">
                        <div className="inline-flex border-2 border-black" role="group" aria-label="Quantity">
                          ...
                          <span className="w-11 h-11 flex items-center justify-center font-[family-name:var(--font-mono)] font-bold border-x-2 border-black">
                            {line.qty}
                          </span>
                          <button
                            type="button"
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            className="font-[family-name:var(--font-mono)] text-xs uppercase tracking-widest underline underline-offset-4 min-h-11 px-2"
                          >
                            Hapus
                          </button>
                        </div>
              ...
              <aside className="border-2 border-black p-6 h-fit lg:sticky lg:top-24">
                <h2 className="font-[family-name:var(--font-display)] uppercase text-2xl mb-4 tracking-tight">Ringkasan</h2>
                <dl className="space-y-3 font-[family-name:var(--font-mono)] text-sm">
                  ...
                  <div className="border-t-2 border-black pt-3 flex justify-between text-base">
                    ...
                    <dd className="font-[family-name:var(--font-display)] uppercase text-2xl tracking-tight">{formatRupiah(subtotal)}</dd>
                  </div>
                </dl>
                <Link href="/checkout" className="btn btn-invert w-full mt-6">Checkout →</Link>
                <Link href="/design-1#shop" className="btn btn-ghost w-full mt-2">Lanjut belanja</Link>
              </aside>
            </div>
          )}
        </div>
