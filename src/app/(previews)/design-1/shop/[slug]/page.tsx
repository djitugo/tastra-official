        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-16 grid lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          >
            <div className="relative aspect-square border-2 border-black bg-white">
              ...
              />
              {off > 0 && (
                <span style={{ transform: "rotate(-10deg)" }} className="absolute top-4 left-4 inline-block bg-black text-white font-[family-name:var(--font-mono)] uppercase text-[11px] font-bold tracking-widest px-3 py-1 border-2 border-black">
                  -{off}% OFF
                </span>
              )}
            </div>
              ...
              <div className="mt-3 grid grid-cols-4 gap-3">
                {product.images.map((src, i) => (
                  <div key={i} className="relative aspect-square border-2 border-black bg-white">
                    <Image src={src} alt={`${product.name} ${i + 1}`} fill sizes="(max-width: 768px) 25vw, 150px" className="object-contain p-2" />
                  </div>
                ))}
              </div>
            )}
          ...
          >
            <p className="font-[family-name:var(--font-mono)] text-xs uppercase tracking-widest text-black/60">
            ...
            <div className="mt-6 border-2 border-black p-4">
              ...
              <ol className="space-y-2 text-sm list-decimal list-inside marker:font-[family-name:var(--font-mono)] marker:font-bold">
                ...
                return (
                  <Link key={p.slug} href={`/design-1/shop/${p.slug}`} className="group block border-2 border-black bg-white transition-transform hover:-translate-y-1 hover:[box-shadow:6px_6px_0_0_#000]">
                    <div className="relative aspect-square border-b-2 border-black bg-white overflow-hidden">
                      <Image src={p.images[0]} alt={p.name} fill sizes="25vw" className="object-contain p-6" />
